...
public class SoldProductJpaResource {
	...
	public ResponseEntity<Void> addToBasket(@PathVariable String username, @RequestBody SoldProduct soldProduct)
			throws URISyntaxException {
		SoldProduct newSoldProduct = soldProductJpaRepository.save(soldProduct);
		URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(newSoldProduct.getId())
				.toUri();
